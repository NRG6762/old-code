 ...
 */
@Autonomous(name="Full Speed Ahead", group="Testing")
//@Disabled
public class BuilderAutonomous100Percent extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if((pureOrient - lastOrient) > (Math.PI * 2 - .5)){
                rotationCount++;
            }else if((pureOrient - lastOrient) < -(Math.PI * 2 - .5)){
                rotationCount--;
            }
    ...
    /*private double[] driveCommand(double startTicks, double currTicks, double goalTicks, double polarAngle, double polarSpeen){

        double finalReturns[] = new double[4];

        double polarSpeed = motorCurve(startTicks, currTicks, goalTicks, .5);
    ...
    private double motorCurve(double startTicks, double currTicks, double goalTicks, double maxPower){
        ...
        return maxPower - maxPower * Math.pow(shiftedCurrTicks - .9 * rangeTicks / 2, 2) / Math.pow(1.1 * rangeTicks / 2, 2);
