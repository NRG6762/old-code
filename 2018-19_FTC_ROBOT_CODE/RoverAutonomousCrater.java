...
//@Disabled
public class RoverAutonomousCrater extends LinearOpMode {
    ...
    private double runtimeHold = 0.0;
    private boolean mineralDrive = false;
    private boolean goldObj = false;
    private String goldPos = "LEFT";
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if(Math.abs(currOrient - pureOrient) > 180){
            ...
            switch (stage) {
                ...
                /*case "Mineral: Move Left to Center Mineral":
                ...
                case "Mineral: Move Left to Left Mineral":
                    speed = driveCurve(2200, robot.drive4.getCurrentPosition(), .8);
                    robot.drive1.setPower(0);
                    robot.drive2.setPower(-speed);
                    robot.drive3.setPower(0);
                    robot.drive4.setPower(speed);
                ...
                case "Mineral: From Hit":
                    speed = driveCurve(400, robot.drive3.getCurrentPosition(), .9);
                    robot.drive1.setPower(-speed);
                    robot.drive2.setPower(0);
                    robot.drive3.setPower(speed);
                    robot.drive4.setPower(0);
                    QuantORun++;
                    /*try {
                        File file = new File("/sdcard/FIRST/blackbox.txt","a");
                        FileOutputStream fileoutput = new FileOutputStream(file);
                        PrintStream ps = new PrintStream(fileoutput);
                        ps.println("Speed: " + speed + "  Encoder " +  robot.drive3.getCurrentPosition());
                        ps.close();
                        fileoutput.close();
                ...
                case "Move Left":
                    if(goldPos.equals("RIGHT")){
                        speed = driveCurve(4000, robot.drive4.getCurrentPosition(), .8);
                    }else if(goldPos.equals("CENTER")){
                        speed = driveCurve(3000, robot.drive4.getCurrentPosition(), .8);
                    }else if(goldPos.equals("LEFT") || goldPos.equals("UNKNOWN")){
                ...
                case "To Depot":
                    speed = driveCurve(4250, robot.drive1.getCurrentPosition(), 1);
                    robot.drive1.setPower(speed);
                    robot.drive2.setPower(0);
                    robot.drive3.setPower(-speed);
                    robot.drive4.setPower(0);
                ...
                case "Drop It":
                    ...
                    stage = "Turn Two Electric Boogaloo";
                    break;

                case "Turn Two Electric Boogaloo":

                    if (currOrient < startOrient + 290) {
                        robot.drive1.setPower(.4);
                        robot.drive2.setPower(.4);
                        robot.drive3.setPower(.4);
                        robot.drive4.setPower(.4);
                ...
                case "Return to Crater":
                    speed = driveCurve(5000, robot.drive3.getCurrentPosition(), 1);
                    robot.drive1.setPower(-speed);
                    robot.drive2.setPower(0);
                    robot.drive3.setPower(speed);
                    robot.drive4.setPower(0);
    ...
    /*public double orientDifference(double currOrient, double startOrient, double difference) {
        double startOrientUse = startOrient;

        if(startOrient - difference <= -180){
            startOrientUse = startOrient + 360;
        }
